/////////////////////////////////////////////////////////////////////////
//
// © University of Southampton IT Innovation Centre, 2014
//
// Copyright in this library belongs to the University of Southampton
// IT Innovation Centre of Gamma House, Enterprise Road,
// Chilworth Science Park, Southampton, SO16 7NS, UK.
//
// This software may not be used, sold, licensed, transferred, copied
// or reproduced in whole or in part in any manner or form or in or
// on any media by any person other than in accordance with the terms
// of the Licence Agreement supplied with the software, or otherwise
// without the prior written consent of the copyright owners.
//
// This software is distributed WITHOUT ANY WARRANTY, without even the
// implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR
// PURPOSE, except where stated in the Licence Agreement supplied with
// the software.
//
//	Created By :			Maxim Bashevoy
//	Created Date :			2014-04-01
//	Created for Project :           EXPERIMEDIA
//
/////////////////////////////////////////////////////////////////////////
package uk.co.soton.itinnovation.ecc.service.domain;

import net.sf.json.JSONObject;

/**
 * Miscellaneous configuration.
 */
public class MiscConfiguration {

    private int snapshotCount;
    private String nagiousUrl;

    public MiscConfiguration() {
    }

    public MiscConfiguration(int snapshotCount, String nagiousUrl) {
        this.snapshotCount = snapshotCount;
        this.nagiousUrl = nagiousUrl;
    }

    public int getSnapshotCount() {
        return snapshotCount;
    }

    public void setSnapshotCount(int snapshotCount) {
        this.snapshotCount = snapshotCount;
    }

    public String getNagiousUrl() {
        return nagiousUrl;
    }

    public void setNagiousUrl(String nagiousUrl) {
        this.nagiousUrl = nagiousUrl;
    }

    /**
     * @return simple net.sf.json.JSONObject representation.
     */
    public JSONObject toJson() {
        JSONObject result = new JSONObject();
        result.put("snapshotCount", snapshotCount);
        result.put("nagiousUrl", nagiousUrl);
        return result;
    }
}
