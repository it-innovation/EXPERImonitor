...
public class ConfigurationService {
    ...
    public static EccConfiguration createDefaultConfiguration(String projectName) {
    ...
    public static boolean validateConfiguration( EccConfiguration config ) throws Exception {
        ...
        final String err = "Cannot validation configuration: ";
        
        if ( config == null ) throw new Exception( err + "config is null" );
        
        // Test essential Rabbit config
        RabbitConfiguration rc = config.getRabbitConfig();
       
        if ( rc == null )                   throw new Exception( err + "rabbit configuration is null" );
        if ( rc.getMonitorId() == null )    throw new Exception( err + "ECC UUID is invalid" );
        if ( rc.getIp() == null )           throw new Exception( err + "Rabbit server IP is null" );
        if ( rc.getPort() == null )         throw new Exception( err + "Rabbit port is null" );
        if ( rc.getUserName() == null )     throw new Exception( err + "Rabbit username is null" );
        if ( rc.getUserPassword() == null ) throw new Exception( err + "Rabbit password is null" );
        
        // Test essential database config
        DatabaseConfiguration dc = config.getDatabaseConfig();
        
        if ( dc == null )                   throw new Exception( err + "database configuration is null" );
        if ( dc.getDatabaseName() == null ) throw new Exception( err + "database name is null" );
        if ( dc.getDatabaseType() == null ) throw new Exception( err + "database type is null" );
        if ( dc.getUrl() == null )          throw new Exception( err + "database url is null" );
        if ( dc.getUserName() == null )     throw new Exception( err + "database username is null" );
        if ( dc.getUserPassword() == null ) throw new Exception( err + "database password is null" );
        
        // Test essential misc config
        MiscConfiguration mc = config.getMiscConfig();
        
        if ( mc == null )                 throw new Exception( err + "Misc configuration is null" );
        if ( mc.getSnapshotCount() < 1 )  throw new Exception( err + "Snapshot count is 0 or less" );
        if ( mc.getNagiousUrl() == null ) throw new Exception( err + "No NAGIOS configuration found" );
    ...
    public void init(String username, String password) throws Exception {
        ...
        // See if it's possible to connect to the configuration service (will throw
        // if there's a connection problem)
        try {
            
            IECCProjectConfig pc = ECCConfigAPIFactory.getProjectConfigAccessor( "default", username, password );
            String url           = pc.getProjectUrl();
            
            configUsername = username;
            configPassword = password;
            
            logger.info( "Successfully retrieved default URL configuration using username and password" );
            
            serviceInitialised = true;
        }
        catch ( Exception ex ) {
            
            String msg = "Could not contact EXPERIMEDIA confirguration service: " + ex.getMessage();
            logger.error( msg );
    ...
    public EccConfiguration getConfiguration(String projectName) {
        ...
        if ( serviceInitialised && projectName != null && !projectName.isEmpty() ) {
    ...
    public void updateConfiguration(String projectName, EccConfiguration config) throws Exception {
        ...
        if ( !serviceInitialised ) throw new Exception( "Could not update configuration: service not initialised" );
