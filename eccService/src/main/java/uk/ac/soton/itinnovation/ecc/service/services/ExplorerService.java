...
public class ExplorerService {
    ...
    public void shutdown() {
        if (metricsQueryHelper != null) {
            metricsQueryHelper.shutdown();
        }
        if (provenanceQueryHelper != null) {
            provenanceQueryHelper.shutdown();
        }
                ...
                for (Attribute attr : sortedAttrs) {
                    ...
                    if (metric != null) {
                        ...
                        switch (metric.getMetricType()) {
                            case NOMINAL:
                            case ORDINAL:
                                result.addQoEAttribute(info);
                                break;

                            case INTERVAL:
                            case RATIO:
                                result.addOtherAttribute(info);
                                break;
                        }
                    } else {
                        logger.warn("Found attribute without metric. Not included in common attribute result set");
                    }
                }
            } catch (Exception e) {
    ...
    public EccParticipantResultSet getPartQoEAttrSelection(UUID expID,
        ...
        if (serviceReady && expID != null && attrName != null && selLabel != null) {
            ...
            if (allPartIRIs == null || allPartIRIs.isEmpty()) {
                return result;
            }
        ...
        if (serviceReady && expID != null && attrName != null) {
            ...
            if (attrResult != null && !attrResult.isEmpty()) {
                ...
                if (ms != null) {
                    ...
                    logger.error("Could not find any measurement sets for attributes by name " + attrName);
                }
            } else {
                logger.warn("Could not find any attributes by name " + attrName);
            }
    ...
    public EccNOMORDParticipantSummary getPartQoEDistribution(UUID expID,
        ...
        if (serviceReady && expID != null && partIRI != null) {
            ...
            if (entity != null) {
                ...
                for (Attribute attr : sortedAttrs) {
                    ...
                    try {
                        ...
                        if (fullSet != null) {
                            ...
                            logger.error("Failed to combine measurement sets: no measurement sets available");
                        }
                    } catch (Exception ex) {
                ...
                logger.warn("Could not find Entity in metric database based on IRI: " + partIRI);
            }
        ...
        if (serviceReady && expID != null) {
            ...
            for (String attrName : attrDistrMap.keySet()) {
                ...
                for (String indLabel : attrValueDistr.keySet()) {
                    ...
                    } else // ... or create one if it does not exist
                    {
    ...
    /**
     * Use this method to retrieve QoS data series of a specific QoS data set
     * and a mapped data series for each participant based on their activities.
     *
                ...
                try {
                    ...
                    if (partIRIs != null && !partIRIs.isEmpty()) {
                        for (String partIRI : partIRIs) {
                            ...
                            if (part != null) {
//                                EccINTRATSeries subSeries = copySeries(part.getName(), true, attrSeries);
                                EccINTRATSeries subSeries = copyMaxValueFromSeries(part.getName(), true, attrSeries);
                                ...
                                logger.warn("Could not retrieve participant by IRI " + partIRI);
                            }
                        }
                    } else {
        ...
        if (attr != null && ms != null) {
            try {
                ...
                if (mt == MetricType.INTERVAL || mt == MetricType.RATIO) {
                    ...
                    logger.error("Could not create INTRAT summary: metric type incorrect");
                }
            } catch (Exception ex) {
    ...
    /**
     * Sets all values in target to max of srcSeries.
     *
     * @param key
     * @param enabled
     * @param srcSeries
     * @return
     */
    private EccINTRATSeries copyMaxValueFromSeries(String key,
            boolean enabled,
            EccINTRATSeries srcSeries) {
        ...
        if (srcMeasures.size() > 0) {
            EccMeasurement maxSrcM = srcMeasures.get(0);
            // find max in srcMeasures
            for (EccMeasurement srcM : srcMeasures) {
                if (Double.parseDouble(srcM.getValue()) > Double.parseDouble(maxSrcM.getValue())) {
                    maxSrcM = srcM;
                }
            }

            // set max to everything in target
            for (EccMeasurement srcM : srcMeasures) {
                targMeasures.add(new EccMeasurement(srcM.getTimestamp(), maxSrcM.getValue()));
            }
        }
    ...
    private void filterSeriesByActivities(EccParticipantActivityResultSet pars,
        ...
        for (int i = 0; i < subMeasures.size() - 1; ++i) {
            ...
            if (switchOff) {
                subMeasures.get(i).setValue(null);
            }
        }
