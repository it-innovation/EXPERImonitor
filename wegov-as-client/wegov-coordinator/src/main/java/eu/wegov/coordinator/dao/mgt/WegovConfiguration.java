 ...
 */
public class WegovConfiguration extends Dao {
    public static final String TABLE_NAME = "Configurations";
    
    public WegovConfiguration() {
        this("", "", "", "");
    }

    public WegovConfiguration(String name, String command, String description, String rendererJsp) {
        super(TABLE_NAME);
        properties.add(new Triplet("ID", "SERIAL PRIMARY KEY", ""));
        properties.add(new Triplet("Name", "character varying(256) NOT NULL", name));
        properties.add(new Triplet("Command", "text", command));
