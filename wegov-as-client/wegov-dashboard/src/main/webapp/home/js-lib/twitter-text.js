...
(function() {
      ...
      if (index % 4 !== 0) {
        ...
        newText += chunk.replace(twttr.txt.regexen.autoLinkUsernamesOrLists, function(match, before, at, user, slashListname, offset, chunk) {
          ...
          if (slashListname && !options.suppressLists) {
            ...
            return stringSupplant("#{before}#{at}<a target=\"_blank\" class=\"#{urlClass} #{listClass}\" href=\"#{listUrlBase}#{list}\"#{extraHtml}>#{chunk}</a>", d);
          } else {
            if (after && after.match(twttr.txt.regexen.endScreenNameMatch)) {
              ...
              return stringSupplant("#{before}#{at}<a target=\"_blank\" class=\"#{urlClass} #{usernameClass}\" #{dataScreenName}href=\"#{usernameUrlBase}#{chunk}\"#{extraHtml}>#{chunk}</a>", d);
            }
          }
        });
      }
  ...
  twttr.txt.autoLinkHashtags = function(text, options) {
    ...
    return text.replace(twttr.txt.regexen.autoLinkHashtags, function(match, before, hash, text) {
      ...
      return stringSupplant("#{before}<a target=\"_blank\" href=\"#{hashtagUrlBase}#{text}\" title=\"##{text}\" class=\"#{urlClass} #{hashtagClass}\"#{extraHtml}>#{hash}#{text}</a>", d);
    });
  };
