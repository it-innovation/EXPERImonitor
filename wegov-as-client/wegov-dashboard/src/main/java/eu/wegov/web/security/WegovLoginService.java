...
public class WegovLoginService {
    ...
    public int saveRunResultsDataAsJson(
        ...
        return helper.getCoordinator().saveRunResultsDataAsJson(
                runId, type, getLoggedInUserAsPolicymaker().getID(), name, location, nResults, minId, maxId, minTs, maxTs, dataAsJson, collected_at);
    }
...
//                toolName = "Groups Search";
                toolName = "Groups Search and Analysis";
            }
            ...
            if (userActivityName != null) {
                /*
                 * if (sites.equals("facebook")) { activityName += " for " +
                 * userActivityName; } else { activityName += " for \"" +
                 * userActivityName + "\""; }
                 */
    ...
    public Activity addActivity2DefaultWorksheet(String activityName, String activityComment, ConfigurationSet confSet, Map<String, String> params) throws Exception {
        ...
        /*
         * //start dev code LOG.info("Getting activity 49"); final Activity act
         * = helper.getCoordinator().getActivityByID(49); final ConfigurationSet
         * cSet = act.getConfigurationSet(); LOG.info("cSet.getConfigurations");
         * final ArrayList < Configuration > cConf = cSet.getConfigurations();
         * LOG.info("Add and clone the Configuration Set retrieved via its ID
         * (done)"); //end dev code
         */
    ...
    public JSONUserDetails getLoggedInUserDetails() {
        try {
            ...
            return new JSONUserDetails(pm.getName(), pm.getRoles().get(0).getName().trim(), pm.getUserName(), pm.getOrganisation(), pm.getID());
            ...
            for (int i = 0; i < numRuns; i++) {
                ...
                if (run.getStatus().equals(Run.STATUS_FINISHED)) {
                    ...
                    if (resultsMetadata == null) {
                        ...
                        /*
                         * int nResults = resultsMetadata.getnResults(); String
                         * type = resultsMetadata.getType(); resultsSummary =
                         * nResults + " " + type;
                         */
    ...
    public boolean ifCredentialsMatch(String username, String password) {
        ...
        try {
            ...
            userTryingToLogin = helper.getCoordinator().getPolicymakerByUsername(username);
