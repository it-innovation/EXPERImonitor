...
public abstract class WeGovAnalysis {
     ...
     *
     * System.out.println("\nStoring results as JSON object"); int wsId = 0; //
     * N/A int runId = Integer.parseInt(tool.getMyRunId()); String type =
     * getSearchType(); String name = getSearchName(); String location =
     * getLocation(); int nResults = getNumResults(); System.out.println("Total
     * number of results: " + nResults); String dataAsJson =
     * getResultsDataAsJson(); Timestamp collected_at = collectionDate; String
     * minId = "0"; String maxId = "0"; Timestamp minTs = "0"; Timestamp maxTs =
     * getMaxTs(); tool.getCoordinator().saveRunResultsDataAsJson(wsId, runId,
     * type, name, location, nResults, minId, maxId, minTs, maxTs, dataAsJson,
     * collected_at); }
     */
    ...
    protected void getInputDataSets(String analysisSubType) throws Exception {
         ...
         * postId}
         *
         * analysis.type analysis.type is either "topic-opinion" or "behaviour"
         * Defined in addTopicOpinionTool and addKMITool in Database maintenance
         *
         * analysis.subType this is either: facebook-post-comments-topics
         * facebook-group-topics twitter-topics twitter-behaviour
         *
         *
         * DATA Types = "posts-facebook" "post-comments-facebook"
         * "posts-twitter"
         *
         * "topic-opinion.twitter-topics" "behaviour.twitter-behaviour"
         ...
         * inputDataAsJSON
         *
         * FacebookPosts: postData query data [] userData []
         *
         * FacebookComments: postData query data [] userData []
         *
         * Tweets: postData query results [] userData []
         *
         */
        ...
        /*
         * JSONObject postData = new JSONObject(); postData.put("query",
         * postId); postData.put("data", commentsJsonArray);
         *
         * resultsData.put("postData", postData); resultsData.put("userData",
         * usersJson);
         */
        for (int k = 0; k < this.rawInputData.size(); k++) {
            //inputDataAsJSON.add (JSONSerializer.toJSON(this.inputData.get(k).getDataAsJson()));
/*
             * // make sure search results are all the same, i.e. the same as
             * the first one String searchResultFirstType =
             * this.inputData.get(0).getType();
             */
            String searchResultType = this.rawInputData.get(k).getType();
            /*
             * if (!searchResultType.equals(searchResultFirstType) { throw new
             * Exception ("Results for activity " + " are not the same!");
             *
             */
             ...
             *
             * // add each element of the current result set for (int m = 0; m <
             * tempDataAsJSONArray.size(); m++) { inputDataAsJSON.add
             * (tempDataAsJSONArray.get(m)); } } else if (tempDataAsJSON
             * instanceof JSONObject) {
             *
             * JSONObject tempDataAsJSONObject = (JSONObject)tempDataAsJSON;
             *
             * inputDataAsJSON.add (tempDataAsJSONObject); } else { throw new
             * Exception ("Data as JSON returned is neither a JSONArray nor a
             * JSONObject"); }
             */
     ...
     *
     * int runId = 1; // get list of runs from configuration // or make list of
     * runs from spec in configuration
     *
     * // Make array list
     *
     ...
     *
     * for (int i = 0; i < runData.size(); i++) { result[i] = new
     * WidgetDataAsJson ( runData.get(i).getId(), runData.get(i).getWidgetId(),
     * runData.get(i).getPmId(), runData.get(i).getActivityId(),
     * runData.get(i).getRunId(), runData.get(i).getType(),
     * runData.get(i).getName(), runData.get(i).getLocation(),
     * runData.get(i).getNumResults(), runData.get(i).getMinId(),
     * runData.get(i).getMaxId(), runData.get(i).getDataAsJson(),
     * df.format(runData.get(i).getTimeCollected()));
     *
     * }
     *
     * //ArrayList<WegovWidgetDataAsJson> widgetDataFromDb =
     * getDataSchema().getAllWhereSortBy(new WegovWidgetDataAsJson(), map,
     * "collected_at");
     *
     * return result; }
     */
    ...
    /*
     * protected WegovWidgetDataAsJson getResultsFromPreviousRun() throws
     * Exception { Run previousRun = tool.getPreviousRun();
     *
     * if (previousRun == null) { System.out.println("No previous run for this
     * activity"); return null; }
     *
     * //Previously we took this from the PostItem table...
     *
     * //WegovPostItem latestPostItem = (WegovPostItem)
     * previousRun.getFirstResult(new WegovPostItem(), "\"ID\""); //return
     * latestPostItem.getID();
     *
     * //Now we assume that this is not available, so we get it from
     * WidgetDataAsJson table WegovWidgetDataAsJson results =
     * tool.getCoordinator().getResultsForRun(previousRun.getID(), false);
     *
     * return results; }
     */
}
