...
public class Model {	
	...
	public boolean loadModel(){
                String modelName = "final";
                try{
                    ModelHandler.loadModel(this, modelName);
                }catch(Exception e){
                    System.out.println("Error while loading model: " + e.getMessage());
                    return false;
                }
	...
	public boolean saveModel(String modelName){
		...
		if (twords > 0){
            ...
            */    
                try{
                    ModelHandler.saveModel(this,modelName);
                }catch(Exception e){
                    System.out.println("Error while saving model to database:" + e.getMessage());
                    e.printStackTrace();
                }
		return true;
	}
	 ...
	 */
	public boolean initNewModel(LDACmdOption option, WegovImporter parent){
		if (!init(option))
			return false;
		...
		data = LDADataset.readDataSet(parent);
		if (data == null){
	 ...
	 */
	public boolean initNewModel(LDACmdOption option, Model trnModel, WegovImporter parent)
	{
		...
		LDADataset dataset = LDADataset.readDataSet(dir + File.separator + dfile, trnModel.data.localDict, parent);
		if (dataset == null){
